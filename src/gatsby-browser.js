...
exports.onInitialClientRender = (_, pluginOptions) => {
  if (!pluginOptions.enableForAllEnvironments && process.env.NODE_ENV !== `production` && process.env.NODE_ENV !== `test`)
    return null;
...
exports.onRouteUpdate = ({ location }, pluginOptions = {}) => {
  if (!pluginOptions.enableForAllEnvironments && process.env.NODE_ENV !== `production` || typeof gtag !== `function`) {
    return null
  }
